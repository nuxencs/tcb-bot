...
RUN go build -ldflags "-s -w -X main.version=${VERSION} -X main.commit=${REVISION} -X main.date=${BUILDTIME}" -o bin/tcb-bot
...
COPY --from=app-builder /src/bin/tcb-bot /usr/bin/

# make folders
RUN mkdir "${APP_DIR}" && \
# create user
    useradd -u 1000 -U -d "${CONFIG_DIR}" -s /bin/false tcb-bot && \
    usermod -G users tcb-bot

WORKDIR /config

ENTRYPOINT ["tcb-bot", "--config", "/config/config.yaml"]
#CMD ["--config", "/config"]